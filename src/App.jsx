                    ...
                    {HABITS.map(h => {
                      ...
                      return (
                        <button key={h.id} onClick={() => !isFuture && toggle(h.id, year, month, day)}
                          ...
                          <span style={{ fontSize: 10, color: done ? h.color : "#999", textDecoration: done ? "line-through" : "none", lineHeight: 1.2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: done ? 500 : 400 }}>{h.label}</span>
                        </button>
                      );
                    })}
                  </div>
